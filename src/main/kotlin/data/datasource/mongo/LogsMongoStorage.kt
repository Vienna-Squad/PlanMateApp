...
class LogsMongoStorage : MongoStorage<Log>(MongoConfig.database.getCollection("Logs")) {
    ...
    fun getLogsByAffectedId(id: UUID): List<Log> {
        return collection.find(Filters.eq("affectedId", id))
            .map { fromDocument(it) }
            .toList()
    }
}