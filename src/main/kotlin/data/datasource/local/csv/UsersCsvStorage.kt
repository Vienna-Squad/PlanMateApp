...
class UsersCsvStorage(
    ...
    override fun deleteItem(item: User) = fileManager.delete(item).let { if (!it) throw DeletionException() }
    override fun getItemById(id: UUID) = fileManager.getById(id) ?: throw UserNotFoundException()
    override fun updateItem(updatedItem: User) = fileManager.update(updatedItem).let { if (!it) throw ModificationException() }
}