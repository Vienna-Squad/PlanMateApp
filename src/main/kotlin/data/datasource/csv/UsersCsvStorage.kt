...
import org.example.domain.UserNotFound
...
class UsersCsvStorage(
    ...
    override fun getAllItems() = fileManager.readAll().ifEmpty { throw NoUsersFound() }
    override fun addItem(newItem: User) = fileManager.append(newItem)
    override fun deleteItem(item: User) = fileManager.delete(item)
    override fun getItemById(id: UUID) = fileManager.getById(id) ?: throw UserNotFound()
    override fun updateItem(updatedItem: User) =
        fileManager.update(updatedItem).let { if (!it) throw UserNotFound() }
}