...
import org.example.domain.NoTasksFound
import org.example.domain.TaskNotFound
...
class TasksCsvStorage(
    ...
    override fun getAllItems() = fileManager.readAll().ifEmpty { throw NoTasksFound() }
    override fun addItem(newItem: Task) = fileManager.append(newItem)
    override fun deleteItem(item: Task) = fileManager.delete(item)
    override fun getItemById(id: UUID) = fileManager.getById(id) ?: throw TaskNotFound()
    override fun updateItem(updatedItem: Task) =
        fileManager.update(updatedItem).let { if (!it) throw TaskNotFound() }
}