...
class TasksCsvStorage(
    ...
    override fun getAllItems() = fileManager.readAll().ifEmpty { throw NotFoundException("tasks") }
    override fun addItem(newItem: Task) = fileManager.append(newItem)
    override fun deleteItem(item: Task) = fileManager.delete(item)
    override fun getItemById(id: UUID) = fileManager.getById(id) ?: throw NotFoundException("task")
    override fun updateItem(updatedItem: Task) =
        fileManager.update(updatedItem).let { if (!it) throw NotFoundException("task") }
}